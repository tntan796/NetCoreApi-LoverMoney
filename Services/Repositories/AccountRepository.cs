        ...
        public async Task<Account> GetAccountById(string id)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    var param = new DynamicParameters();
                    param.Add("@Id", id);
                    var category = await connection.QuerySingleAsync<Account>(storeProcedureName, param, commandType: CommandType.StoredProcedure);
                    return category;
                }
            }
            catch (Exception ex)
        ...
        public ResponseList<IEnumerable<Account>> GetAccounts(FilterBase filter)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    var account = connection.Query<Account>(storeProcedureName, param, commandType: CommandType.StoredProcedure);
                    var result = new ResponseList<IEnumerable<Account>>(account, param.Get<int>("@total"), param.Get<int>("@totalFiltered"));
                    return result;
                }
            }
            catch (Exception ex)
